...
function Write-SimConstantsFile {
    ...
    $cfg = if ($null -ne $parsed.config) { $parsed.config } else { $parsed }

    $mass   = if ($null -ne $cfg.robotMassKg)         { $cfg.robotMassKg }         else { 55.0 }
    $moi    = if ($null -ne $cfg.robotMoiKgM2)        { $cfg.robotMoiKgM2 }        else { 6.0 }
    $width  = if ($null -ne $cfg.bumperWidthMeters)   { $cfg.bumperWidthMeters }   else { 0.9 }
    $length = if ($null -ne $cfg.bumperLengthMeters)  { $cfg.bumperLengthMeters }  else { 0.9 }
